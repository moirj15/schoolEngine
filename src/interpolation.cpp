...
#include <cstdio>
    ...
    //                 1.0f,  0.0f,  0.0f, 0.0f};
template <typename T>
static std::array<T, 4> CreateControlPoints(const T &start, const T &p, 
        const T &end) {
...
glm::quat DCCSpline(const f32 u, const KeyFrame &start, const KeyFrame &p, const KeyFrame &end) {
    ...
    const auto q0 = glm::slerp(controlPoints[0], controlPoints[1], u);
    const auto q1 = glm::slerp(controlPoints[1], controlPoints[2], u);
    const auto q2 = glm::slerp(controlPoints[2], controlPoints[3], u);
    const auto r0 = glm::slerp(q0, q1, u);
    const auto r1 = glm::slerp(q1, q2, u);
